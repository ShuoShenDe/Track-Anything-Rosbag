...
def get_cvmat_by_sam():
    ...
    source = json_data.get('source',
                           "/home/ubuntu/Documents/EFS/Personal/ShuoShen/Track-Anything-Rosbag/test_sample/denso_tri52_seg_pre_labeling.bag")
    ...
    print("tracking_frame_start is {} tracking_frame_end is {}".format(int(tracking_frame_start)-1, int(tracking_frame_end)-1))
    # cv_mat[cv_mat == 1] = 100
    cv2.imwrite('single_channel_image.png', cv_mat)
    ...
    current_time = datetime.datetime.now()
    formatted_time = current_time.strftime("%Y-%m-%d %H:%M:%S")
    print("finish tracking time is: {}".format(formatted_time))
    ...
    for frame_array in masks:
        ...
        image_path = os.path.join("/home/ubuntu/Documents/EFS/Personal/ShuoShen/Track-Anything-Rosbag", file_name_with_timestamp)
        ...
        frame_id = frame_id + 1
        # os.remove(file_name_with_timestamp)
        # if frame_id >= 2:
        #     break
